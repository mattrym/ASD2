    ...
    public static Graph Lab03Reverse(this Graph g)
        {
			...
			Graph ng = g.IsolatedVerticesGraph();
			for (int v = 0; v < n; ++v)
				foreach (Edge e in g.OutEdges(v))
					ng.AddEdge(e.To, e.From, e.Weight);
			return ng;
        }
    ...
    public static bool Lab03IsBipartite(this Graph g, out int[] vert)
        {
			if (g.Directed)
				throw new Lab03Exception();
			int[] col = new int[g.VerticesCount];  // zmienić
			int color = 1, cc;

			Predicate<int> vv = delegate (int v)
			{
				col[v] = color;
				color = color == 1 ? 2 : 1;
				return true;
			};
			Predicate<Edge> ee = delegate (Edge e)
			{
				return !(col[e.From] == col[e.To]);
			};
			
			bool result = g.GeneralSearchAll<EdgesStack>(vv, ee, out cc);
			vert = result ? col : null;
    ...
    public static Graph Lab03Kruskal(this Graph g, out int mstw)
        {
			...
			int from, to;
			int n = g.VerticesCount, weight=0;
			...
			n--;
			while(!empq.Empty && t.EdgesCount<n)
			{
				Edge e = empq.Get();
				from = e.From;
				to = e.To;
    ...
    public static bool Lab03IsUndirectedAcyclic(this Graph g)
        {
			if (g.Directed)
				throw new Lab03Exception();
			int cc;
			g.GeneralSearchAll<EdgesQueue>(null, null, out cc); // zmienić
